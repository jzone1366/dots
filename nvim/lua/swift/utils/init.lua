...
function M.get_bufnrs()
  local bufnrs = vim.tbl_filter(function(bufnr)
    ...
    if vim.bo[bufnr].filetype == 'swiftterm' then
      return false
    end
...
function M.hl.extend(target, source, opts)
  M.hl.set(target, vim.tbl_extend('force', M.hl.get(source), opts or {}))
end
...
function M.hl.adopt_winhighlight(win_id, target, name, fallback)
  ...
  local found = M.find(parts, function(part)
...
local function convert_hl_to_val(opts)
  for name, value in pairs(opts) do
    if type(value) == 'table' and value.from then
      opts[name] = M.hl.get_hl(value.from, vim.F.if_nil(value.attr, name))
    end
  end
end
...
function M.hl.plugin(name, hls)
  ...
  M.all(hls)
  swift.augroup(fmt('%sHighlightOverrides', name), {
    {
      event = { 'ColorScheme' },
      command = function()
        M.all(hls)
