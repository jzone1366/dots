            ...
            default_on_attach(client, bufnr, function()
              if string.match(vim.fn.expand('%:p:h'), '_notes') then
                vim.keymap.set('n', '<leader>ff', function()
                  swift.picker.find_files({ cwd = vim.g.notes_path })
                end, { desc = '[f]ind in [n]otes', buffer = bufnr })

                vim.keymap.set(
                  'n',
                  'gd',
                  '<cmd>lua vim.lsp.buf.definition()<CR>',
                  { desc = '[g]o to note [d]efinition', noremap = true, buffer = bufnr }
                )

                vim.keymap.set('n', 'g.', function()
                  local note_title = require('swift.utils').notes.get_md_link_dest()
