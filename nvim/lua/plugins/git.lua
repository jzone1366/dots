...
return {
  ...
  },
  {
    ...
    config = function()
      ...
      require('swift.autocmds').augroup('Neogit', {
  ...
  },
  {
    ...
    config = function()
      ...
      require('swift.autocmds').augroup('GitConflicts', {
        {
          event = { 'User' },
          ...
          command = function(args)
            ...
            nnoremap(
              ...
              { desc = 'git-conflict: conflicts in qf', buffer = args.buf }
            )
            nnoremap(
              'cq',
              '<cmd>GitConflictListQf<CR>',
            ...
            vim.defer_fn(function()
              ...
              vim.diagnostic.hide()
              swift.notify(string.format('%s Conflicts detected.', icons.lsp.error))
            end, 250)
          end,
        },
        {
          ...
          command = function(args)
            vim.defer_fn(function()
              ...
              local ok, gd = pcall(require, 'garbage-day.utils')
              if ok then
                gd.start_lsp(gd.stop_lsp())
              end
              vim.diagnostic.show()
              swift.notify(string.format('%s All conflicts resolved!', icons.lsp.ok))
              if pcall(require, 'fidget') then
    ...
    keys = {
      {
        '<leader>gww',
        function()
          require('telescope').extensions.git_worktree.git_worktrees(swift.picker.dropdown({ path_display = {} }))
        end,
        desc = 'git-worktree: switch worktree',
      },
      {
