...
return {
  {
    ...
    init = function()
      local function terminal_notifier(term_cmd, exit)
        if exit == 0 then
          swift.notify(fmt('👍 [PASS] vim-test: %s', term_cmd), L.INFO)
          -- vim.fn.system(string.format([[terminal-notifier -title "nvim [test]" -message "👍 test(s) passed"]], term_cmd))
        else
          swift.notify(fmt('👎 [FAIL] vim-test: %s', term_cmd), L.ERROR)
      ...
      vim.g['test#custom_strategies'] = {
        termsplit = function(cmd)
          swift.term(term_opts(cmd))
        end,
        termvsplit = function(cmd)
          if vim.opt.lines:get() * 4 < vim.opt.columns:get() then
            swift.term(term_opts(cmd, { direction = 'vertical', size = 100 }))
          else
            swift.term(term_opts(cmd))
          end
        end,
        termfloat = function(cmd)
          swift.term(term_opts(cmd, { direction = 'float', focus_on_open = true }))
        end,
        termtab = function(cmd)
          swift.term(term_opts(cmd, { direction = 'tab', focus_on_open = true }))
        end,
      }
  ...
  },
  {
    ...
    config = function()
      ...
      function nt.read_env(...)
        nt.env = vim.fn.DotenvRead(...)
      end
