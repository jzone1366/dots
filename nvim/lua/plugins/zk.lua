    ...
    local function on_attach(client, bufnr)
      if client.name == 'zk' and require('zk.util').notebook_root(vim.fn.expand('%:p')) ~= nil then
        ...
        local function pick(opts)
          local default_style = swift.picker.ivy
        ...
        -- COMMANDS
        -- ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
        command('ZkToday', function(_)
          zkc('ZkNew', { dir = 'log/daily' })
        end, { desc = 'zk: opens or creates zk note for today' })
        command('ZkLog', function(_)
          get_notes({ hrefs = { 'log' }, sort = { 'created' } })
        end, { desc = 'zk: list all the zk log notes' })
        command('ZkMultiTags', function(opts)
          get_tags(opts, pick({ title = 'zk multi tags' }), function(tags)
            ...
            get_notes(
              { tags = tags },
              pick({ title = 'zk notes for tag(s) ' .. vim.inspect(tags), multi_select = true }),
              function(notes)
                local cmd = 'args'
        ...
        vim.keymap.set(
          ...
          { desc = '[g]oto [d]efinition (follow link)', noremap = false }
        )
