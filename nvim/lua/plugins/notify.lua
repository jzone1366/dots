...
return {
  {
    ...
    event = 'VeryLazy',
    cond = vim.g.notifier_enabled and vim.g.notifier == 'mini',
    config = function()
  ...
  },
  {
    ...
    event = 'VeryLazy',
    cond = vim.g.notifier_enabled and vim.g.notifier == 'nvim-notify',
    config = function()
      ...
      local override = function(msg, level, opts)
        ...
        if nok then
          if opts.title and U.starts_with(opts.title, '[LSP]') then
