return {
  {
    ...
    config = function()
      ...
      require('swift.autocmds').augroup('Telescope', {
        {
      ...
      swift.picker.dropdown = dropdown
      ...
      local function grep(opts)
        ...
        current_fn = fn
        -- opts.cwd = require("swift.utils").get_root()
      ...
      swift.picker.find_files = find_files
      ...
      swift.picker.startup = function(bufnr)
        swift.picker.find_files({
          theme = 'dropdown',
          hidden = true,
          no_ignore = false,
          previewer = false,
      ...
      if vim.g.picker == 'telescope' then
        ...
        map('n', '<leader>ff', function()
          swift.picker.find_files({ picker = 'smart_open', theme = 'ivy' })
        end, '[f]ind [f]iles')
        ...
        map('n', '<leader>fg', function()
          swift.picker.grep({ picker = 'egrepify' })
        end, { '[f]ind e[g]repify' })
        ...
        map('n', '<leader>a', swift.picker.grep, { 'grep (live)' })
        -- map("n", "<leader>A", ts.grep_string, {  "grep (under cursor)" })
        map('n', '<leader>A', function()
          swift.picker.grep({ default_text = vim.fn.expand('<cword>') })
        end, { 'grep (under cursor)' })
        map({ 'v', 'x' }, '<leader>A', function()
          local pattern = require('swift.utils').get_visual_selection()
          swift.picker.grep({ default_text = pattern })
        end, { 'grep (selection)' })

        map('n', '<leader>fu', ts.undo, { '[f]ind [u]ndo' })
        -- map("n", "<leader>fd", ts.diagnostics, {  "[f]ind [d]iagnostics" })
        map('n', '<leader>fd', function()
          swift.picker.find_files({ picker = 'smart_open', cwd = vim.g.dotfiles_path })
        end, { '[f]ind in [d]otfiles' })
        map('n', '<leader>fc', function()
          swift.picker.find_files({ picker = 'smart_open', cwd = vim.fn.stdpath('config') })
        end, { '[f]ind in [c]onfig' })
        map('n', '<leader>fp', function()
          swift.picker.find_files({ picker = 'smart_open', cwd = vim.fn.expand(vim.g.code_path), title = 'in ~/code' })
        ...
        map('n', '<leader>fn', function()
          swift.picker.find_files({ picker = 'smart_open', cwd = vim.g.notes_path })
        end, { '[f]ind in [n]otes' })
