...
local augroup = require('swift.autocmds').augroup
    ...
    config = function()
      ...
      local function go_to_unique_definition()
        vim.lsp.buf_request(0, 'textDocument/definition', vim.lsp.util.make_position_params(), function(_, result, _, _)
          ...
          if #unique_results == 1 then
            ...
            swift.blink_cursorline(100, true)
          else
      ...
      function M.on_attach(client, bufnr, cb)
        ...
        if client and client.supports_method('textDocument/inlayHint', { bufnr = bufnr }) then
          if SETTINGS.enabled_inlay_hints[filetype] then
        ...
        local nmap = function(keys, func, d)
          map('n', keys, func, { buffer = bufnr, desc = desc(d), noremap = false })
        end
        local vnmap = function(keys, func, d)
          map({ 'v', 'n' }, keys, func, { buffer = bufnr, desc = desc(d) })
        end
        ...
        if client.name == 'elixirls' then
          vim.keymap.set(
            ...
            '<localleader>efp',
            ':ElixirFromPipe<cr>',
            { buffer = bufnr, noremap = true, desc = 'from pipe' }
          )
          vim.keymap.set(
            'n',
            '<localleader>etp',
            ':ElixirToPipe<cr>',
            { buffer = bufnr, noremap = true, desc = 'to pipe (|>)' }
          )
          vim.keymap.set(
            'v',
            '<localleader>eem',
            ':ElixirExpandMacro<cr>',
            { buffer = bufnr, noremap = true, desc = 'expand macro' }
          )
        end
        ...
        command('LspCapabilities', function(ctx)
          ...
          local clientInfo = vim.tbl_map(function(c)
            return c.name .. '\n' .. vim.inspect(c)
          end, clients)
        ...
        local function diag_source_as_suffix(diag, mode)
          if not (diag.source or diag.code) then
            return '', ''
          end
        ...
        vim.diagnostic.config({
          ...
          float = {
            ...
            suffix = function(diag)
              return diag_source_as_suffix(diag, 'float')
            end,
        ...
        local ns = vim.api.nvim_create_namespace('swift_lsp_diagnostics')
        ...
        if vim.tbl_contains(SETTINGS.max_diagnostic_exclusions, client.name) then
          ...
          vim.diagnostic.handlers.signs = vim.tbl_extend('force', orig_signs_handler, {
            ...
            hide = function(_, bn)
              orig_signs_handler.hide(ns, bn)
            end,
          })
        end

        -- require("swift.lsp_diagnostics")
        ...
        augroup('LspProgress', {
          {
            ...
            command = function(evt)
              if pcall(require, 'fidget') then
                ...
                if evt.data.result and evt.data.result.token then
                  token = evt.data.result.token
                end
                local client_id = evt.data.client_id
                local c = client_id and vim.lsp.get_client_by_id(client_id)
                if c and token then
                  require('fidget').notification.remove(c.name, token)
                end
              end
            end,
          },
      ...
      local servers = require('swift.servers')
      if servers == nil then
        return
      end
